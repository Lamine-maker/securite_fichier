# aes.py
# Implémentation AES-128 from scratch (ECB et CFB)
# Interface IDENTIQUE à ton ancien code

BLOCK_SIZE = 16

# =========================
# Tables AES
# =========================

S_BOX = [
    
]

INV_S_BOX = [S_BOX.index(i) for i in range(256)]

RCON = [
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
]

# =========================
# Outils internes
# =========================

def pad(data: bytes) -> bytes:
    padding_len = BLOCK_SIZE - (len(data) % BLOCK_SIZE)
    return data + bytes([padding_len] * padding_len)

def unpad(data: bytes) -> bytes:
    return data[:-data[-1]]

def xor(a, b):
    return bytes(i ^ j for i, j in zip(a, b))

def bytes2matrix(data):
    return [list(data[i:i+4]) for i in range(0, 16, 4)]

def matrix2bytes(matrix):
    return bytes(sum(matrix, []))

def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = S_BOX[state[i][j]]

def inv_sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = INV_S_BOX[state[i][j]]

def shift_rows(state):
    state[1] = state[1][1:] + state[1][:1]
    state[2] = state[2][2:] + state[2][:2]
    state[3] = state[3][3:] + state[3][:3]

def inv_shift_rows(state):
    state[1] = state[1][-1:] + state[1][:-1]
    state[2] = state[2][-2:] + state[2][:-2]
    state[3] = state[3][-3:] + state[3][:-3]

def xtime(a):
    return ((a << 1) ^ 0x1B) & 0xFF if a & 0x80 else a << 1

def mix_columns(state):
    for i in range(4):
        a = state[i]
        t = a[0] ^ a[1] ^ a[2] ^ a[3]
        u = a[0]
        a[0] ^= t ^ xtime(a[0] ^ a[1])
        a[1] ^= t ^ xtime(a[1] ^ a[2])
        a[2] ^= t ^ xtime(a[2] ^ a[3])
        a[3] ^= t ^ xtime(a[3] ^ u)

def inv_mix_columns(state):
    for i in range(4):
        a = state[i]
        u = xtime(xtime(a[0] ^ a[2]))
        v = xtime(xtime(a[1] ^ a[3]))
        a[0] ^= u
        a[1] ^= v
        a[2] ^= u
        a[3] ^= v
    mix_columns(state)

# =========================
# Key schedule
# =========================

def expand_key(master_key):
    key_columns = bytes2matrix(master_key)
    i = 1
    while len(key_columns) < 44:
        word = list(key_columns[-1])
        if len(key_columns) % 4 == 0:
            word = word[1:] + word[:1]
            word = [S_BOX[b] for b in word]
            word[0] ^= RCON[i]
            i += 1
        word = [w ^ k for w, k in zip(word, key_columns[-4])]
        key_columns.append(word)
    return [key_columns[i:i+4] for i in range(0, 44, 4)]

# =========================
# AES bloc
# =========================

def encrypt_block(block, round_keys):
    state = bytes2matrix(block)
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_keys[0][i][j]

    for r in range(1, 10):
        sub_bytes(state)
        shift_rows(state)
        mix_columns(state)
        for i in range(4):
            for j in range(4):
                state[i][j] ^= round_keys[r][i][j]

    sub_bytes(state)
    shift_rows(state)
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_keys[10][i][j]

    return matrix2bytes(state)

def decrypt_block(block, round_keys):
    state = bytes2matrix(block)
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_keys[10][i][j]

    for r in range(9, 0, -1):
        inv_shift_rows(state)
        inv_sub_bytes(state)
        for i in range(4):
            for j in range(4):
                state[i][j] ^= round_keys[r][i][j]
        inv_mix_columns(state)

    inv_shift_rows(state)
    inv_sub_bytes(state)
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_keys[0][i][j]

    return matrix2bytes(state)

# =========================
# API PUBLIQUE (identique)
# =========================

def aes_encrypt(data: bytes, key: bytes, mode: str) -> bytes:
    key = key.ljust(16, b'\0')[:16]
    round_keys = expand_key(key)

    if mode.upper() == "ECB":
        data = pad(data)
        return b"".join(encrypt_block(data[i:i+16], round_keys)
                        for i in range(0, len(data), 16))

    elif mode.upper() == "CFB":
        iv = b"\x00" * 16
        result = b""
        for i in range(0, len(data), 16):
            stream = encrypt_block(iv, round_keys)
            block = data[i:i+16]
            cipher = xor(block, stream[:len(block)])
            result += cipher
            iv = cipher
        return result

    else:
        raise ValueError(f"Mode AES inconnu: {mode}")

def aes_decrypt(data: bytes, key: bytes, mode: str) -> bytes:
    key = key.ljust(16, b'\0')[:16]
    round_keys = expand_key(key)

    if mode.upper() == "ECB":
        plain = b"".join(decrypt_block(data[i:i+16], round_keys)
        for i in range(0, len(data), 16))
        return unpad(plain)

    elif mode.upper() == "CFB":
        iv = b"\x00" * 16
        result = b""
        for i in range(0, len(data), 16):
            stream = encrypt_block(iv, round_keys)
            block = data[i:i+16]
            plain = xor(block, stream[:len(block)])
            result += plain
            iv = block
        return result

    else:
        raise ValueError(f"Mode AES inconnu: {mode}")
